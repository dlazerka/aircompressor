...
class SequenceEncoderBb {
    ...
    private SequenceEncoderBb() {
    }

    public static int compressSequences(
            ByteBuffer outputBase,
            final int outputAddress,
            int outputSize,
            SequenceStoreBb sequences,
            CompressionParameters.Strategy strategy,
            SequenceEncodingContext workspace
    ) {
        int output = outputAddress; // TODO: change to outputBase.position()
        ...
        if (sequenceCount < 0x7F) {
            ...
            outputBase.put(output, (byte) sequenceCount);
            output++;
            ...
            outputBase.put(output, (byte) (sequenceCount >>> 8 | 0x80));
            outputBase.put(output + 1, (byte) sequenceCount);
            output += SIZE_OF_SHORT;
        } else {
            // UNSAFE.putByte(outputBase, output, (byte) 0xFF);
            outputBase.put(output, (byte) 0xFF);
            output++;
        ...
        int literalsLengthEncodingType = selectEncodingType(
                largestCount,
                sequenceCount,
                DEFAULT_LITERAL_LENGTH_NORMALIZED_COUNTS_LOG,
                true,
        ...
        int offsetEncodingType = selectEncodingType(
                largestCount,
                sequenceCount,
                DEFAULT_OFFSET_NORMALIZED_COUNTS_LOG,
                defaultAllowed,
        ...
        int matchLengthEncodingType = selectEncodingType(
                largestCount,
                sequenceCount,
        ...
        //         outputBase,
        //         headerAddress,
        //         (byte) ((literalsLengthEncodingType << 6) | (offsetEncodingType << 4) | (matchLengthEncodingType << 2))
        // );
        outputBase.put(
                headerAddress,
                (byte) ((literalsLengthEncodingType << 6) | (offsetEncodingType << 4) | (matchLengthEncodingType << 2))
    ...
    private static int buildCompressionTable(
        ...
        return FiniteStateEntropy.writeNormalizedCounts(
                ...
                maxSymbol,
                tableLog
        ); // TODO: pass outputLimit directly
    }
    ...
    private static int encodeSequences(
        ...
        if (sequenceCount >= 2) {
            for (int n = sequenceCount - 2; n >= 0; n--) {
                ...
                literalLengthState = literalLengthTable.encode(
                        blockStream,
                        literalLengthState,
                        literalLengthCode
                ); // 33
