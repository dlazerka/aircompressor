...
class ZstdFrameDecompressorBb
{
    ...
    static FrameHeader readFrameHeader(ByteBuffer bb)
    {
        int inputAddress = bb.position();

        // long input = inputAddress;
        // verify(input < inputLimit, input, "Not enough input bytes");
        verify(bb.hasRemaining(), bb.position(), "Not enough input bytes");

        // int frameHeaderDescriptor = UNSAFE.getByte(inputBase, input++) & 0xFF;
        int frameHeaderDescriptor = bb.get() & 0xFF;
        ...
        verify(headerSize <= bb.remaining(), bb.position(), "Not enough input bytes");

        // decode window size
        int windowSize = -1;
        if (!singleSegment) {
            // int windowDescriptor = UNSAFE.getByte(inputBase, input++) & 0xFF;
            int windowDescriptor = bb.get() & 0xFF;
    ...
    static void verifyMagic(ByteBuffer bb)
    {
        ...
        if (magic != MAGIC_NUMBER) {
            if (magic == V07_MAGIC_NUMBER) {
                throw new MalformedInputException(bb.position(), "Data encoded in unsupported ZSTD v0.7 format");
            }
